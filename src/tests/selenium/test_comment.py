...
class TestCommentsAndAnswers:
    ...
    def test_user_read_answers_of_main_comment(self, selenium, wait_driver, live_server, main_comment_with_answers):
        main_comment, answers = main_comment_with_answers
        # user enters to the site
        selenium.get(live_server.url)
        ...
        answer_elements = wait_comment.until(
            ec.visibility_of_all_elements_located((By.CSS_SELECTOR, '#answer_block div[data-comment-type="answer"]'))
        )
        assert len(answer_elements) == 25

        # user scrolls to down and click on show comments yet link
        show_comments_yet_link = wait_comment.until(ec.element_to_be_clickable((By.ID, 'show_comments_yet')))
